...
# -*- coding: utf-8 -*-
"""
Launcher RPA - Órdenes de Venta
Sistema automatizado para procesamiento de órdenes de venta en SAP Business One
"""
...
class LauncherVentas:
    def __init__(self):
        try:
            # Configurar la ventana principal
            self.root = tk.Tk()
            self.root.title("AI4U | RPA - Órdenes de Venta")
            ...
            # Log inicial
            self.log_message("🚀 Launcher de Órdenes de Venta iniciado")
    ...
    def create_widgets(self):
        ...
        try:
            ...
            title_label = ttk.Label(
                title_frame,
                text="AI4U | Sistema RPA - Órdenes de Venta",
                font=("Arial", 16, "bold")
            )
            ...
            subtitle_label = ttk.Label(
                title_frame,
                text="Automatización de órdenes de venta en SAP Business One | hola@ai4u.com.co",
                font=("Arial", 10)
            )
            ...
            left_frame = ttk.LabelFrame(main_frame, text="CONFIGURACIÓN DE VENTAS", padding=12)
            left_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0, 6))
            
            self._create_sales_info(left_frame)
    ...
    def _create_sales_info(self, parent):
        """Crea el panel de información de ventas"""
        try:
            # Título
            title_label = ttk.Label(parent, text="📋 Módulo de Órdenes de Venta", font=("Arial", 12, "bold"))
            title_label.pack(pady=(0, 12))
            
            # Información del módulo
            info_text = """
🎯 FUNCIONALIDADES:
• Carga automática del NIT del cliente
• Ingreso del número de orden de compra
• Procesamiento de items y cantidades
• Navegación automática en SAP
• Captura de screenshots de confirmación
• Integración con Google Drive

📁 ARCHIVOS PROCESADOS:
• Formato JSON con datos de órdenes
• Ubicación: data/outputs_json/sales_order/
...
• Archivos JSON en formato correcto
            ...
            self.show_error_dialog("Error creando panel de ventas", str(e))
    ...
    def _create_control_panel(self, parent):
        ...
        try:
            ...
            self.start_btn = ttk.Button(
                parent,
                text="🚀 INICIAR PROCESAMIENTO DE VENTAS",
    ...
    def _create_status_bar(self):
        ...
        try:
            self.status_bar = ttk.Label(
                self.root,
                text="Sistema de Órdenes de Venta - Listo",
                relief=tk.SUNKEN,
                anchor=tk.W
            )
    ...
    def start_sales_processing(self):
        """Inicia el procesamiento de órdenes de venta"""
        try:
            if self.is_running:
                messagebox.showwarning("Sistema en ejecución", "El sistema ya está procesando órdenes de venta.")
                return
            
            self.is_running = True
            self.start_btn.config(state=tk.DISABLED)
            self.stop_btn.config(state=tk.NORMAL)
            self.status_label.config(text="Estado: Procesando órdenes de venta...")
            self.status_bar.config(text="Procesando órdenes de venta...")
            
            self.log_message("🚀 Iniciando procesamiento de órdenes de venta...")
            
            # Iniciar procesamiento en hilo separado
            processing_thread = threading.Thread(target=self._run_sales_processing, daemon=True)
    ...
    def _run_sales_processing(self):
        """Ejecuta el procesamiento de órdenes de venta"""
        try:
            # Importar el módulo de ventas
            from rpa.modules.sales_order.sales_order_handler import SalesOrderHandler
            
            self.log_message("📋 Importando módulo de órdenes de venta...")
            ...
            self.log_message("Verificar que el módulo sales_order esté disponible")
        except Exception as e:
    ...
    def show_configuration(self):
        ...
        try:
            config_path = os.path.join(project_root, "rpa", "modules", "sales_order", "sales_order_config.yaml")
            ...
            if os.path.exists(config_path):
                ...
                config_window.title("Configuración de Órdenes de Venta")
    ...
    def show_pending_files(self):
        ...
        try:
            pending_dir = os.path.join(project_root, "data", "outputs_json", "sales_order", "01_Pendiente")
